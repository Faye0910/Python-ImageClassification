...
#============== 載入影像資料庫 ===============================
# fd:欲載入的影像的目錄  pre_adjust:是否做前置影像調整
def load_dataset(fd, pre_adjust=True):  
    if(fd=='MNIST'): #如果是預設的MNIST影像
        (_, _), (imgs, labs) = mnist.load_data()         
    else: #自訂的影像，載入影像與標籤，並做適當的影像處理
        imgs, labs = read_from_fd(fd, False, pre_adjust) 
        
    print(fd,': 影像的shape=',imgs.shape,', 標籤的shape=',labs.shape)    
    return imgs, labs
...
def read_from_fd(path, pre_adjust=True):
    test_images=[]  # 建立空的list
    test_labels=[]  # 建立空的list  

    if pre_adjust:
        print(" [影像調整] ",end='')       
                
    for label in range(10):  #數字0-9
        files = glob.glob(path + "/" + str(label) + "/*.png" ) #files儲存所有影像的檔名    
...
def pre_proc(imgs, labs, model):
    if imgs != np.ndarray:  #如果imgs不是ndarray的型別，則轉為numpy 的array形別
        imgs = np.array(imgs)
